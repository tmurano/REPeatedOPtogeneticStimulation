...
# FC Context Distance
############################################
Data <- read_excel("FearConditioning_Data.xlsx", sheet = "2.Context_Dist")
...
df <- data.frame(
  Time = rep(1:5, 3),
  StimType = rep(c("NoStim", "Stimx3+2wks", "Stimx10+2wks"), each = 5),
...
g <- ggplot(df, aes(x = Time, y = Mean, color = StimType, group = StimType)) +
  ...
  scale_y_continuous(expand = c(0, 0), breaks = seq(0, 500, 100), limits = c(0, 500)) +
  xlab("Time (min)") +
  ylab("Distance traveled (cm)")

ggsave("Context_d1_Distance.png", g, width = 6, height = 6, dpi = 300)

############################################
# FC Context Distance - Statistics
############################################
Data_transposed <- as.data.frame(t(Data))
colnames(Data_transposed) <- c(
  paste0("NoStim_", 1:10),
  paste0("Stim3_", 1:8),
  paste0("Stim10_", 1:10)
)

Data_transposed$timebin <- 1:5
...
write.xlsx(
  ...
  "Context_d1_Distance_Stats.xlsx"
)

############################################
# FC Context Freezing
############################################
Data <- read_excel("FearConditioning_Data.xlsx", sheet = "2.Context_Freezing")
rownames(Data) <- rowname
Data <- Data[new_order, ]
Data <- Data[-c(18, 20), -1] # remove dead mouse
...
ggsave("Context_d1_Freezing.png", g, width = 6, height = 6, dpi = 300)

############################################
# FC Context Freezing - Statistics
############################################
Data_transposed <- as.data.frame(t(Data))
colnames(Data_transposed) <- c(
  paste0("NoStim_", 1:10),
...
write.xlsx(
  list(ANOVA_Result = anova, Bonf_Result1 = pwc1, Bonf_Result2 = pwc2),
  "Context_d1_Freezing_Stats.xlsx"
)
