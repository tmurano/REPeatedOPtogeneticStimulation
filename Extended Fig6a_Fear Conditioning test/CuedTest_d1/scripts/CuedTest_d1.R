...
############################################
# FC 1st Day Conditioning Distance
############################################
Data <- read_excel("FearConditioning_Data.xlsx", sheet = "3.Cued_Dist")
...
ggsave("Cued_d1_Distance.png", g, width = 6, height = 6, dpi = 300)

############################################
# FC 1st Day Conditioning Distance - Statistics
############################################
...
write.xlsx(
  ...
  file = "Cued_d1_Distance_Stats.xlsx"
)

############################################
# FC 1st Day Conditioning Freezing
############################################
Data <- read_excel("FearConditioning_Data.xlsx", sheet = "3.Cued_Freezing")
Data <- Data[-c(18, 20), -1] # remove dead mouse
...
ggsave("Cued_d1_Freezing.png", g, width = 6, height = 6, dpi = 300)

############################################
# FC 1st Day Conditioning Freezing - Statistics
############################################
Data_transposed <- as.data.frame(t(Data))
colnames(Data_transposed) <- c(NoStim, Stim3[1:8], Stim10)
Data_transposed$timebin <- 1:6
...
write.xlsx(
  list(ANOVA_Result = anova, Bonf_Result1 = pwc1, Bonf_Result2 = pwc2),
  file = "Cued_d1_Freezing_Stats.xlsx"
)
